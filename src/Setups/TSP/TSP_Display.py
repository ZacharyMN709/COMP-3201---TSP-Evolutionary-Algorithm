...
class GraphingHelper:
    ...
    def read_tsp_file(self, fnum):
        if fnum == 1:
            fname = os.path.join('Sahara', "TSP_WesternSahara_29.txt")
        elif fnum == 2:
            fname = os.path.join('Uruguay', "TSP_Uruguay_734.txt")
        elif fnum == 3:
            fname = os.path.join('Canada', "TSP_Canada_4663.txt")
        else:
            print('Warning! Invalid seletion. Defaulting to Sahara')
            fname = os.path.join('', "TSP_WesternSahara_29.txt")
